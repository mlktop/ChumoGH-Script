...
make install
badvpn-udpgw --listen-addr 127.0.0.1:7200 --max-connections-for-client 1000 --client-socket-sndbuf 0 --udp-mtu 9000 > /dev/nul &
echo 'badvpn-udpgw --listen-addr 127.0.0.1:7200 --max-connections-for-client 1000 --client-socket-sndbuf 0 --udp-mtu 9000 > /dev/nul &' > /etc/rc.local
#echo 'badvpn-udpgw --listen-addr 127.0.0.1:7200 > /dev/nul &' >> /etc/rc.local
cd /etc/adm-lite/
rm -f badvpn-1.999.128.tar.*
rm -f install > /dev/nul
echo "Finalizado"
echo "Modulo BadUDP - FocoeFe"
